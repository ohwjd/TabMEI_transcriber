...
cp_dirs = ["java/utils/lib/*", "java/utils/bin/"]
...
def _handle_scoreDef(
    scoreDef: ET.Element, ns: dict, uri: str, args: argparse.Namespace
):  # -> None
    """
    ...
    if args.tab == "y":
        ...
        if not_type != notationtypes["GLT"]:
            tab_staffDef.set(
                "lines", "5" if lines == "5" and args.type == "FLT" else "6"
            )
        ...
        for i, (pitch, octv) in enumerate(tunings[args.tuning]):
            course = ET.SubElement(
                ...
                oct=str(octv),
                accid=(
                    "" if len(pitch) == 1 else ("f" if pitch[1] == "b" else "s")
                ),
            )
    # Remove
    else:
    ...
    for i in [1] if args.staff == "s" else [1, 2]:
        nh_staffDef = ET.SubElement(
            nh_staffGrp, uri + "staffDef", n=str(i), lines="5"
        )
        if i == 1:
            nh_staffDef.set("dir.dist", "4")
        # Add <clef>
        if args.staff == "s":
            clef = _create_element(
                uri + "clef",
                parent=nh_staffDef,
                atts=[
                    ("shape", "G"),
...
def _handle_section(
    section: ET.Element, ns: dict, uri: str, args: argparse.Namespace
):  # -> None
    """
    ...
    for measure in section.iter(uri + "measure"):
        ...
        tab_staff.set(
            "n",
            str(int(tab_staff.attrib["n"]) + (1 if args.staff == "s" else 2)),
        )
        ...
        for tabGrp in tab_layer.iter(uri + "tabGrp"):
            ...
            if len(tabGrp) == 1 and flag != None:
                _create_element(
                    uri + "rest",
                    parent=nh_layer_1,
                    atts=[("dur", dur), ("xml:id", xml_id)],
                )
                _create_element(
                    uri + "rest",
                    parent=nh_layer_2,
                    atts=[("dur", dur), ("xml:id", xml_id + "_lwr")],
                )
                # Add <dir>
                measure.insert(
                    len(measure) - 1, _make_dir(uri, xml_id, dur, dots)
                )
            # Add <chord>s
            else:
                ...
                for element in tabGrp:
                    if element != flag:
                        ...
                        if midi_pitch_class in mpcGrid and not any(
                            accidsInEffect
                        ):
                        ...
                        #    is not in key
                            ...
                            accidsInEffect = spell_dict[
                                "accidsInEffect"
                            ]  # list
                        nh_note = _create_element(
                            uri + "note",
                            parent=(
                                chord_1
                                if args.staff == "s"
                ...
                if args.staff == "s":
                    ...
                    if len(chord_1) > 0 and len(chord_2) > 0:
                        ...
                        space = _create_element(
                            uri + "space",
                            atts=[("dur", dur), ("xml:id", xml_id + "_spc")],
                        )
                        nh_layer_1.append(
                            chord_1 if len(chord_1) > 0 else space
                        )
                        nh_layer_2.append(
...
def _make_dir(uri: str, xml_id: str, dur: int, dots: int):  # -> 'ET.Element'
    d = ET.Element(uri + "dir", place="above", startid="#" + xml_id)
    _create_element(
        uri + "symbol",
        parent=d,
        atts=[
...
def transcribe(
    infile: str, arg_paths: dict, args: argparse.Namespace
):  # -> None
    print("infile: ", infile)
    print("arg_paths: ", arg_paths)
    print("args: ", args)
